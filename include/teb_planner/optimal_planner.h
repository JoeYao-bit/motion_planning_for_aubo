 ...
 */
     ...
     */
    virtual bool plan(const std::vector<PoseSE2>& initial_plan, const Eigen::Vector3d start_vel = Eigen::Vector3d(), bool free_goal_vel=false);
     ...
     */
    void getVelocityProfile(std::vector<Eigen::Vector3d>& velocity_profile) const;
   ...
   */
    void getFullTrajectory(std::vector<PoseSE2>& trajectory, std::vector<double>& time_diffs) const;
...
protected:
    ...
    std::pair<bool, Eigen::Vector3d> vel_start_; //!< Store the initial velocity at the start pose
    std::pair<bool, Eigen::Vector3d> vel_goal_; //!< Store the final velocity at the goal pose
...
#endif //FREENAV_OPTIMAL_PLANNER_H
