...
typedef std::function<double(const PoseSE2&, const PoseSE2&)> DIST_TO_OBSTACLE_FUNC;

class EdgeObstacleGridMap : public BaseTebMultiEdge<1, double>
{
public:
    ...
    EdgeObstacleGridMap()
    {
        this->resize(2); // consider line dist to obstacle
        _measurement = NULL;
    }
    ...
    void computeError()
    {
        ...
        const VertexPose* pt1 = static_cast<const VertexPose*>(_vertices[0]);
        const VertexPose* pt2 = static_cast<const VertexPose*>(_vertices[1]);

        double dist = dist_func_(pt1->pose(), pt2->pose());
