...
bool TimedElasticBand::initTrajectoryToGoal(const std::vector<PoseSE2>& plan, double max_vel_x, double max_vel_theta, bool estimate_orient, int min_samples, bool guess_backwards_motion)
{
    ...
    if (!isInit())
    {
        ...
        for (int i=1; i<(int)plan.size()-1; ++i)
        {
            ...
            if (estimate_orient)
            {
                ...
                double dx = plan[i+1].position().x() - plan[i].position().x();
                double dy = plan[i+1].position().y() - plan[i].position().y();
                yaw = std::atan2(dy,dx);
                if (backwards)
            ...
            PoseSE2 intermediate_pose(plan[i].position().x(), plan[i].position().y(), yaw);
    ...
    else // size!=0
    {
        ...
        printf("Number of TEB configurations: %d, Number of TEB timediffs: %d /n", sizePoses(), sizeTimeDiffs());
        return false;
    }
...
{
    if(dt <= 0) {
        std::cerr << "Adding a timediff requires a positive dt" << std::endl;
    }
...
void TimedElasticBand::addPoseAndTimeDiff(double x, double y, double angle, double dt)
{
    if (sizePoses() != sizeTimeDiffs())
    {
        ...
        std::cerr << "Method addPoseAndTimeDiff: Add one single Pose first. Timediff describes the time difference between last conf and given conf" << std::endl;
    return;
}
